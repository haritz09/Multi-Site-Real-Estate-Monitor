...
function openDb() {
    ...
    if (!url || !authToken) {
        throw new Error('Faltan TURSO_DATABASE_URL o TURSO_AUTH_TOKEN en milestone6/.env');
    }
    ...
    for (const item of listings) {
        ...
        if (!current) {
            changesLog.push({
                listing_id: item.id,
                change_type: 'new',
                diff_json: JSON.stringify({ old_price: null, new_price: normPriceStr })
            });
            ...
            if (current.active === 0) {
                ...
                if (priceDiffs.length > 0) {
                    changesLog.push({
                        ...
                        diff_json: JSON.stringify({ old_price: current.price, new_price: normPriceStr })
                    });
                }
                if (attrDiffs.length > 0) {
                    changesLog.push({
                        ...
                        // just save whatever attrDiffs array if we want, or a simplified object
